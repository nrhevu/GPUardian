...
// Repair stale claimed aggregates without guessing completion from age. Claims
// with no job evidence are left alone; any unfinished linked job keeps a claim
// active. Restore primary associations too for databases created by older code.
const migrationV9 = `
INSERT OR IGNORE INTO job_sessions(node_id,job_id,session_id)
  SELECT node_id,job_id,session_id FROM jobs;
WITH aggregates AS (
...
DELETE FROM history_daily_summaries;
`
